        ...
        <li v-for="(course, index) in table.data" :key="index">
          ...
          <div class="flex1">
            ...
            <div>
              ...
              <br>
              <span class="mr-2"><b>Ngày thích:</b> {{ convertTSToDateTime(course.liked_at) }}</span>
            </div>
          </div>
        </li>
      </ul>
...
<script>
import { getListFavoriteCourse } from '../../../api/user'
export default {
  props: {
    elmntId: {
      type: Number,
  ...
  methods: {
    ...
    async getDataList(page = 1) {
      ...
      const { list, total } = await getListFavoriteCourse(this.elmntId, params)
