...
import { addTeacher, getTeacherInfo, updateTeacher, activeTeacher } from '../../../api/teacher'
...
export default {
  ...
  watch: {
    async $route(to, from) {
      if (this.$route.params.id) {
        ...
        this.teacher.date_of_birth = convertTimeStamptoDate(this.teacher.date_of_birth)
      }
    }
  },
  async created() {
  ...
  methods: {
    ...
    onConfirmActive: async function() {
      ...
      try {
        await activeTeacher(this.teacher.id)
