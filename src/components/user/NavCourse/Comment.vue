<template>
  <div class="mt-3 mb-5 us_list_rating">
    <form v-if="this.$store.state.User.myInfo" ref="form" style="text-align: right; display: flex;" @submit.stop.prevent="handleSubmit">
      <b-form-textarea
        id="textarea-large"
...
export default {
  ...
  data() {
    return {
      ...
      comment: {
        ...
        userId: this.$store.state.User.myInfo ? this.$store.state.User.myInfo.id : 0,
        adminId: null,
        teacherId: null,
        role: 'user'
      }
  ...
  methods: {
    ...
    async getDataList(page = 1) {
      ...
      const { list, total } = await getCommentByCourse(params, this.data)
