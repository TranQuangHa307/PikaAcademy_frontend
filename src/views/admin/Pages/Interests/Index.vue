  ...
  <div>
    <div class="table-users">
      ...
      <table cellspacing="0" style="border-bottom: 1px solid #FEC415;">
        ...
        <tbody>
          <tr v-for="(interests, index) in table.data" :key="index">
            <td>{{ interests.id }}</td>
            <td>{{ interests.name }}</td>
            <td>{{ interests.description }}</td>
            <td> <b-img v-if="interests.url_image" :src="interests.url_image" /></td>
            <td style="width: 120px;">
              <div>
                <button
                  class="btn btn-info mr-1 size_75"
                  @click="onOpenModal(false, interests)"
                ><b-icon icon="pencil-fill" variant="white" /></button>
                <button class="btn btn-danger size_75" @click.prevent="onDeleteData(interests)"><b-icon icon="trash-fill" variant="white" /></button>
              </div>
            </td>
          </tr>
        </tbody>
...
<script>
import { getInterestsList, createInterests, updatedInterests, deleteInterests } from '../../../../api/interests'
import ModalFormInterests from '../../../../components/admin/modal/ModalFormInterests.vue'
export default {
  components: {
    modalFormInterest: ModalFormInterests
  },
  data() {
    return {
      checked: true,
  ...
  methods: {
    ...
    async getDataList(page = 1) {
      ...
      const { list, total } = await getInterestsList(params)
